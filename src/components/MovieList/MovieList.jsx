...
const MovieList = ({ movieList, handleLoadMoreMovies, listType }) => {
  return (
    <div className="bg-white flex flex-row flex-wrap pt-4 justify-between space-x-1">
      {movieList.map((movie) => (
        <MovieItem key={movie.id} props={movie} />
      ))}

      <button
        className="focus:outline-none w-full h-16 bg-blue-300 rounded-2xl text-white font-bold text-xl mb-4"
